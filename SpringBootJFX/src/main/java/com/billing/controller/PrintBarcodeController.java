...
public class PrintBarcodeController extends AppContext implements TabContent {
	...
	public void initialize() {
		...
		try {
			imageView.setImage(new Image(new FileInputStream("src\\main\\resources\\images\\65_Labels.png")));
	...
	private void populateImageView() {
		...
		try {
			imageView.setImage(new Image(new FileInputStream("src\\main\\resources\\images\\"+imageName)));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			logger.error(e.getMessage());;
		}
	...
	void onGenerateBarcodeCommand(ActionEvent event) {
		if (txtName.getText().equals("") && !txtBarcode.getText().equals("")) {
			...
			int noOfLabels = 0;
				try {
					...
					boolean isSuccess = jasperUtils.createPDFForBarcode(jasperService.createDataForBarcode(barcode,noOfLabels,startPosition), JrxmlName, fileName);
					if(!isSuccess) {
						alertHelper.showErrorNotification("Barcode length should be 12 digits! Please correct barcode number");
					}
				} catch (Exception e1) {
					e1.printStackTrace();
