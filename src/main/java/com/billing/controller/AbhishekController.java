...
@Controller
public class AbhishekController implements TabContent {
	...
	private void printDonationReceipt() {
		printerService.printAbhishekReceipt(txnDetails);
	}
	...
	public void putFocusOnNode() {
		...
		txtMobile.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent ke) {
				if (!txtMobile.getText().equalsIgnoreCase("") && ke.getCode().equals(KeyCode.ENTER)) {
		...
		txtName.setOnKeyPressed(new EventHandler<KeyEvent>() {
			@Override
			public void handle(KeyEvent ke) {
				if (!txtName.getText().equalsIgnoreCase("") && ke.getCode().equals(KeyCode.ENTER)) {
	...
	private TransactionDetails getTransactionDetails() {
		...
		donation.setCategory(2);
		donation.setCategoryName("अभिषेक पावती");
	...
	public void initialize() {
		...
		txtName.textProperty().addListener(this::invalidated);
