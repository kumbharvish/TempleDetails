...
@Repository
public class TempleDetailsRepository {

	@Autowired
	DBUtils dbUtils;

	private static final Logger logger = LoggerFactory.getLogger(TempleDetailsRepository.class);

	private static final String UPDATE_STORE_DETAILS = "UPDATE TEMPLE_DETAILS SET NAME=?,"
	...
	private static final String INS_STORE_DETAILS = "INSERT INTO TEMPLE_DETAILS (STORE_ID,NAME,ADDRESS,ADDRESS2,CITY,DISTRICT,"
			+ "STATE,PHONE,CST_NUMBER,PAN_NUMBER,VAT_NUMBER,ELECTRICITY_NO,OWNER_NAME,MOBILE_NO,GST_NO)"
			+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	private static final String GET_TEMPLE_DETAILS = "SELECT * FROM TEMPLE_DETAILS";

	public TempleDetails getMyStoreDetailsFromDB() {
		Connection conn = null;
		PreparedStatement stmt = null;
		TempleDetails myStoreDetails = null;
		try {
			conn = dbUtils.getConnection();
			stmt = conn.prepareStatement(GET_TEMPLE_DETAILS);
			ResultSet rs = stmt.executeQuery();

			if (rs.next()) {
				myStoreDetails = new TempleDetails();
	...
	public StatusDTO updateStoreDetails(TempleDetails myStoreDetails) {
		Connection conn = null;
		PreparedStatement stmt = null;
	...
	// Save My Store Details
	public StatusDTO addStoreDetails(TempleDetails myStoreDetails) {
		Connection conn = null;
		PreparedStatement stmt = null;
		StatusDTO status = new StatusDTO();
